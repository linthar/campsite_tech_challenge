...
echo "Deleting previous REDIS server instance on localhost  (if there is one)"
docker stop redisCampsite  > /dev/null 2>&1
docker rm redisCampsite  > /dev/null 2>&1

echo "creating the REDIS Server for campsite service"
docker run -d -p 6379:6379 --name redisCampsite redis
